...
include_once ($_SERVER['DOCUMENT_ROOT'].'/shop/delivery_code.php');
// 테마에 orderinquiryview.php 있으면 include
if (defined('G5_THEME_SHOP_PATH')) {
//	/var/www/html/gvmp/theme/vexmall/mobile/shop/orderinquiryview.php
    ...
    <div id="sod_fin">
                        ...
                        for ($k = 0; $opt = sql_fetch_array($res); $k++) {
                            ...
                            <div class="li_op_wr">
                                ...
                                <a href="./item.php?it_id=<?php echo $row['it_id']; ?>"
                                   class="total_img"><?php echo $image; ?></a>
                                <span class="prqty_stat"><span
                                            class="sound_only">상태</span><?php echo $opt['ct_status']; ?></span>
            ...
            <div class="sod_ta_wr">
                <dl id="m_sod_bsk_tot">
                    ...
                    <?php if ($od['od_cart_coupon'] > 0) { ?>
                        ...
                        <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_cart_coupon']); ?> 원</strong>
                        </dd>
                    <?php } ?>
                    ...
                    <?php if ($od['od_send_cost'] > 0) { ?>
                        ...
                        <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_cost']); ?> 원</strong>
                        </dd>
                    <?php } ?>

                    <?php if ($od['od_send_coupon'] > 0) { ?>
                        <dt class="sod_bsk_dvr">배송비할인</dt>
                        <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_coupon']); ?> 원</strong>
                        </dd>
                    <?php } ?>

                    <?php if ($od['od_send_cost2'] > 0) { ?>
                        <dt class="sod_bsk_dvr">추가배송비</dt>
                        <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_send_cost2']); ?> 원</strong>
                        </dd>
                    <?php } ?>

                    <?php if ($od['od_cancel_price'] > 0) { ?>
                        <dt class="sod_bsk_dvr">취소금액</dt>
                        <dd class="sod_bsk_dvr"><strong><?php echo number_format($od['od_cancel_price']); ?> 원</strong>
                        </dd>
                    <?php } ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if ($od['od_cash']) {
                                            ...
                                            ?>
                                            <a href="javascript:;" onclick="<?php echo $cash_receipt_script; ?>">현금영수증
                                                확인하기</a>
                                            <?php
                                        } else {
                                            ?>
                                            <a href="javascript:;"
                                               onclick="window.open('<?php echo G5_SHOP_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');">현금영수증을
                                                발급하시려면 클릭하십시오.</a>
                                        <?php } ?>
                                    </td>
                                </tr>
            ...
            <ul>
                ...
                <li id="alrdy">
                    ...
                    <?php if ($od['od_receipt_point']) {    //포인트로 결제한 내용이 있으면 ?>
                        <div class="right">
                            <p><span class="title"><i class="fa fa-angle-right"
                                                      aria-hidden="true"></i> 포인트 결제</span><?php echo number_format($od['od_receipt_point']); ?>
                                점</p>
                            <p><span class="title"><i class="fa fa-angle-right"
                                                      aria-hidden="true"></i> 실결제</span><?php echo number_format($od['od_receipt_price']); ?>
                                원</p>
                        </div>
                    <?php } ?>
                </li>
        ...
        <section id="sod_fin_cancel">
            ...
            if ($cancel_price == 0) {
                if ($custom_cancel) {
                    ?>
                    <button type="button" onclick="document.getElementById('sod_fin_cancelfrm').style.display='block';">
                        주문 취소하기
                    </button>

                    <div id="sod_fin_cancelfrm">
                        <form method="post" action="<?php echo G5_SHOP_URL; ?>/orderinquirycancel.php"
                              onsubmit="return fcancel_check(this);">
                            <input type="hidden" name="od_id" value="<?php echo $od['od_id']; ?>">
                            <input type="hidden" name="token" value="<?php echo $token; ?>">

                            <label for="cancel_memo" class="sound_only">취소사유</label>
                            <input type="text" name="cancel_memo" id="cancel_memo" required class="frm_input required"
                                   maxlength="100" placeholder="취소사유">
        ...
        <?php if ($od['od_settle_case'] == '가상계좌' && $od['od_misu'] > 0 && $default['de_card_test'] && $is_admin && $od['od_pg'] == 'kcp') {
            ...
            <div id="kcp_acc_test" class="odf_tbl">
                <form method="post" action="http://devadmin.kcp.co.kr/Modules/Noti/TEST_Vcnt_Noti_Proc.jsp"
                      target="_blank">
                    <table>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="noti_url"
                                       value="<?php echo G5_SHOP_URL; ?>/settle_kcp_common.php"></td>
                        </tr>
                        </tbody>
                    </table>
...
<?php
if ($od['od_invoice'] && $od['od_delivery_company']) {
    $code = delivery_code($od['od_delivery_company']);
    echo '  <form hidden name="sform" id="sform" action="" method="post">
              <input type="text" class="form-control" id="t_key" name="t_key" value="sZigmIK36UFp1QBu50UHrw">
              <input type="text" class="form-control" name="t_code" id="t_code" value="' . $code . '">
              <input type="text" class="form-control" name="t_invoice" id="t_invoice" value="' . $od['od_invoice'] . '">
        </form>';
}
?>
    <script>
        function frm_search() {
            var gsWin = window.open('about:blank', 'payviewer', 'width=450,height=800');
