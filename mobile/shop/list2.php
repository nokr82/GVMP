...
?>
    ...
    <div id="sct">
        ...
        if (!is_file($cate_skin))
            $cate_skin = G5_MSHOP_SKIN_PATH . '/listcategory.skin2.php';
...
//        echo $sql;
        ...
        if (!$ca['ca_mobile_skin']) {
            $ca['ca_mobile_skin'] = '0716list.10.skin.php';
        }
        ...
        // /var/www/html/gvmp/theme/everyday/mobile/skin/shop/basic/list.10.skin.php
        //    $skin_file = is_include_path_check($skin_dir.'/'.$ca['ca_mobile_skin']) ? $skin_dir.'/'.$ca['ca_mobile_skin'] : $skin_dir.'/0716list.10.skin.php';
        $skin_file = $_SERVER['DOCUMENT_ROOT'].'/theme/everyday/mobile/skin/shop/basic/0716list.10.skin.php';
...
//            echo $sql;
            ...
            $sql = "select count(*) as cnt from `temp_item_shop` where it_1 = '' and it_use = '1' and ( ca_id2 like '{$_GET['ca_id']}%' {$where}) order by it_order, it_id desc";
            $query = sql_query($sql);
            $sms_code = sql_fetch_array($query);
            // where 된 전체 상품수
            $total_count = $sms_code['cnt'];
...
//            echo $items;
        ...
        $qstr1 .= 'ca_id=' . $ca_id;
        $qstr1 .= '&amp;qto=' . $_GET['qto'] . '&amp;qfrom=' . $_GET['qfrom'] . '&amp;q=' . $_GET['q'];
