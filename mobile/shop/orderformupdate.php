...
else if ($od_settle_case == "KAKAOPAY")
{
    ...
    if($od_misu == 0)
        $od_status      = '입금';
} else if($od_settle_case == "vmp_point") {
    $od_tno             = $tno;
    $od_receipt_price   = $amount;
    $od_receipt_point   = $i_temp_point;
    $od_receipt_time    = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})/", "\\1-\\2-\\3 \\4:\\5:\\6", $app_time);
...
// VMC, VMP 포인트를 사용했다면 보유 포인트에서 차감시키기
if( isset($_POST['vmcF']) || isset($_POST['vmpF']) ) {
    if( (int)$_POST['vmcF'] > 0 || (int)$_POST['vmpF'] > 0 ) {
		if($_SESSION['app']){
	        ...
	        sql_query("update g5_member set VMC = VMC - {$_POST['vmcF']}, VMP = VMP - {$_POST['vmpF']} where mb_id = '{$member['mb_id']}'");
	        sql_query("insert into dayPoint set mb_id = '{$member['mb_id']}', VMC = -{$_POST['vmcF']}, VMR = 0, VMP = -{$_POST['vmpF']}, date = NOW(), way = 'shopPoint'");
		}
    }
}
...
/*각 상품의 VD에게*/
	...
	$sql = "SELECT *, (SELECT mb_id FROM {$g5['g5_shop_item_table']} WHERE it_id = gc.it_id) AS vd_mb_id FROM {$g5['g5_shop_cart_table']} AS gc WHERE gc.od_id = '".$od_id."'";
	$result = sql_query($sql);
