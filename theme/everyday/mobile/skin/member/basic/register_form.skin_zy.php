...
<div>
    ...
    function Check(x,y) {
        for(i=1;i<=2;i++) { // 라디오 버튼의 갯수
            z = "option" + i ; // name 부분
            document.getElementById(z).src = 'http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif';
        }
       
        document.getElementById(x).src = 'http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton_checked.gif';
    ...
    function Check2(x,y) {
        for(i=4;i<=6;i++) { // 라디오 버튼의 갯수
            z = "option" + i ; // name 부분
                document.all[z].src = "http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" ; // 라디오 버튼 off
        }
            document.all[x].src = "http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton_checked.gif"; // 라디오 버튼 on

        if( x == "option5" ) {
        	$('#hide_area').css('display', 'none');
        	document.getElementById("radiobtn").value = '3';
       ...
       function emailConf() {
        ...
        // 계좌 인증을 진행했는 지 체크하는 로직
//        var data = "bankName=" + $("#bankName option:selected").val() + "&"
//        + "accountNumber=" + $('#reg_mb_account').val();
        
//        $.ajax({
//               url:'/myOffice/accountCheckDB2.php',
//               type:'POST',
//               data: encodeURI(data),
//               async : false,
//               success:function(result){
//                   if( result == "false" ) {
//                       alert("계좌 인증을 진행해 주세요.");
//                       check = "false";
//                   }
//                  }
//          });
...
//        });
    ...
    <form name="fregisterform" id="fregisterform" onsubmit="return fregisterform_submit(this);" action="<?php echo $register_action_url ?>" method="post" enctype="multipart/form-data" autocomplete="off">
        
    <input type="hidden" id="accountType" name="accountType" value="VM">
        
    <input type="hidden" id="vmc" name="vmc" value="<?php echo $_GET['vmc'] ?>">
    <input type="hidden" id="vmr" name="vmr" value="<?php echo $_GET['vmr'] ?>">
    <input type="hidden" id="vmp" name="vmp" value="<?php echo $_GET['vmp'] ?>">
    <input type="hidden" id="loginID" name="loginID" value="<?php echo $_GET['loginID'] ?>">
    ...
    <div class="form_01">
        ...
        VM계정신청 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton_checked.gif" name="option4" onclick="Check2(this.name,'')">
<!--        VD계정신청 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" name="option6" onclick="Check2(this.name,'')">
        CU계정신청 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" name="option5" onclick="Check2(this.name,'')">-->
        <input type="hidden" id="radiobtn2" name="radiobtn2" value="0">
        <br><br>
        
        <h2 style="background:#f2a80b; display: none;">추천인 / 후원인 정보입력</h2>
        <li>
            <label for="reg_mb_recommender" class="sound_only">추천인<strong>필수</strong></label>
            <input type="text" value="<?=$member['mb_name']?>" name="reg_mb_recommender" id="reg_mb_recommender" class="frm_input full_input <?php echo $required ?>" minlength="2" maxlength="20" <?php echo $required ?>  placeholder="추천인" readonly="readonly">
        </li>
        <li>
            <label for="reg_mb_recommender_no" class="sound_only">추천인 회원번호<strong>필수</strong></label>
            <input onkeydown="onlyNumber(this)" value="<?=$member['mb_id']?>" maxlength="8" type="text" name="reg_mb_recommender_no" id="reg_mb_recommender_no" class="frm_input full_input <?php echo $required ?>" minlength="3" maxlength="20" <?php echo $required ?>  placeholder="추천인 회원번호" readonly="readonly">
            
        </li>
        <div id="hide_area">
        <li>
            <label for="reg_mb_sponsor" class="sound_only">후원인<strong>필수</strong></label>
            <input type="text" value="<?=$_GET['mb_name']?>" name="reg_mb_sponsor" id="reg_mb_sponsor" class="frm_input full_input <?php echo $required ?>" minlength="3" maxlength="20" <?php echo $required ?>  placeholder="후원인" readonly="readonly">
        </li>
        <li>
            <label for="reg_mb_sponsor_no" class="sound_only">후원인 회원번호<strong>필수</strong></label>
            <input onkeydown="onlyNumber(this)" value="<?=$_GET['mb_id']?>" maxlength="8" type="text" name="reg_mb_sponsor_no" id="reg_mb_sponsor_no" class="frm_input full_input <?php echo $required ?>" minlength="3" maxlength="20" <?php echo $required ?>  placeholder="후원인 회원번호" readonly="readonly">
        </li>
        ...
        1팀 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" name="option1" id="option1" onclick="Check(this.id,'')">
        2팀 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" name="option2" id="option2" onclick="Check(this.id,'')">
        <!-- 3팀 <img src="http://www.blueb.co.kr/SRC/javascript/image1/forms/radiobutton.gif" name="option3" onclick="Check(this.name,'')"> -->
        <input type="hidden" id="radiobtn" name="radiobtn" value="<?=$_GET['team']?>">
        </div>
        </div>
    </div>
    ...
    <div class="form_01">
        ...
        <li class="rgs_name_li">
            ...
            <input type="text" id="reg_mb_name" name="mb_name" value="" <?php echo $required ?> <?php echo $readonly; ?> class="frm_input full_input <?php echo $required ?> <?php echo $readonly ?>" placeholder="이름">
        ...
        <li class="reg_hp_box">
            ...
            <p class="reg_hp_info">휴대전화 번호 입력후 인증문자 발송 버튼을 누르세요. 4자리 인증번호 문자를 보내드리겠습니다. </p>
            <label for="reg_mb_hp" class="sound_only">휴대폰번호<?php if ($config['cf_req_hp']) { ?><strong>필수</strong><?php } ?></label>
            <input onkeydown="onlyNumber(this)" type="text" name="mb_hp" value="<?php // echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="frm_input full_input required <?php echo ($config['cf_req_hp'])?"required":""; ?>" minlength="11" maxlength="11" title="휴대전화 번호" placeholder="휴대전화 번호를 숫자만 입력" required>
        ...
        <li>
            ...
            <input onkeydown="onlyNumber(this)" maxlength="6" type="text" name="mb_birth" value="" id="reg_mb_tel" class="frm_input full_input required" maxlength="20" placeholder="생년월일 (ex. 890803)">
        </li>

        <?php if ($config['cf_use_addr']) { ?>
        <li>
            <span class="frm_label" style="background:#2399f1;">주소<?php if ($config['cf_req_addr']) { ?><strong class="sound_only">필수</strong><?php } ?></span>
...
<!--        <li>
        ...
        <li>
            ...
            <input type="text" name="mb_accountHolder" id="reg_mb_payment" class="frm_input full_input <?php echo $required ?>" minlength="3" maxlength="20" <?php echo $required ?> placeholder="예금주">
        </li>
        <li>
            <label for="reg_mb_account" class="sound_only">계좌번호<strong>필수</strong></label>
            <input type="text" name="mb_accountNumber" id="reg_mb_account" class="frm_input full_input <?php echo $required ?>" minlength="3" maxlength="20" <?php echo $required ?> placeholder="계좌번호 (숫자만 입력)" onkeydown="onlyNumber(this)">
        </li>
<!--        <li>
...
//        window.open('http://vmptomcat.cafe24.com/account_popup_nh.jsp?'+url, "", "width=500, height=500, left=500, top=100");
//    }
...
function reconfirmHp(){
    ...
    <div class="form_01 agree">
        ...
        <li class="mailing_con">
            <label for="reg_mb_mailling" class="frm_label mailing">메일링서비스</label>
        ...
        <li class="sms_con">
            <label for="reg_mb_sms" class="frm_label sms">SMS 수신여부</label>
    ...
    function fregisterform_submit(f){
        ...
        if ( $('#reg_mb_password').val().length > 0) {
