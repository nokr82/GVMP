...
<section id="bo_vc">
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        <header>
            ...
            <!--중고장터 일 경우 작성자 아이디와 작성일 출력--> 
            <?php if( $_GET['bo_table'] == "used" ){ ?>
            <?php
                $idCheck = mysql_fetch_array(mysql_query("select * from g5_member where mb_id = '".$list[$i]['wr_name']."'"));
            ?>
            <span class="name"><span class="sound_only">작성자</span><?php echo $idCheck["mb_name"] ?>(<?php echo $list[$i]['name'] ?>)</span>
