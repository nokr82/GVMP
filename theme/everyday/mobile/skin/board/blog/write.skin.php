...
add_stylesheet('<link rel="stylesheet" href="write_skin.css">', 1);
?>
<link rel="stylesheet" href="http://gvmp.company/theme/everyday/mobile/skin/board/blog/write_skin.css">
...
<section id="bo_w">
    ...
    <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);"
        ...
        if ($is_notice || $is_html || $is_secret || $is_mail) {
            $option = '';
            if ($is_notice) {
                if ($board['bo_table'] != 'free' && $board['bo_table'] != 'youtube') {
        ...
        <div class="bo_w_info write_div">
            <?php if ($is_name) { ?>
                ...
                <input type="text" name="wr_name" value="<?php echo $name ?>" id="wr_name" required
                       class="frm_input required" placeholder="이름">
            <?php } ?>

            <?php if ($is_password) { ?>
                <label for="wr_password" class="sound_only">비밀번호<strong>필수</strong></label>
                <input type="password" name="wr_password" id="wr_password" <?php echo $password_required ?>
                       class="frm_input <?php echo $password_required ?>" placeholder="비밀번호">
            <?php } ?>

            <?php if ($is_email) { ?>
                <label for="wr_email" class="sound_only">이메일</label>
                <input type="text" name="wr_email" value="<?php echo $email ?>" id="wr_email" class="frm_input email "
                       placeholder="이메일">
            <?php } ?>
        </div>
        ...
        <?php if ($is_homepage) { ?>
            <div class="write_div">
                ...
                <input type="text" name="wr_homepage" value="<?php echo $homepage ?>" id="wr_homepage"
                       class="frm_input full_input" size="50" placeholder="홈페이지">
            </div>
        <?php } ?>
        ...
        <div class="bo_w_link write_div">
            <label for="wr_link<?php echo $i ?>"><i class="fa fa-youtube-play" aria-hidden="true"></i><span
                        class="sound_only">동영상<?php echo $i ?></span></label>
            <?php if ($board['bo_table'] != 'free') {
                echo ' <input type="text" name="wr_10" value="' . $write['wr_10'] . '" id="wr_10" class="frm_input full_input" size="50" placeholder="예) https://www.youtube.com/watch?v=9bZkp7q19f0">';
            } ?>
        ...
        <?php
        if ($board['bo_table'] != 'youtube') {
            for ($i = 0; $is_file && $i < $file_count; $i++) { ?>
                <div class="bo_w_flie write_div">
                    <div class="file_wr write_div">
                        <label for="bf_file_<?php echo $i + 1 ?>" class="lb_icon"><i class="fa fa-download"
