...
FROM
    ...
    where mb_id = '{$member['mb_id']}' and am.menu_category = 1";
    
    $admin_menu = sql_query($admin_sql);
     $menu['menu100'] = array();
    array_push(  $menu['menu100'], array('100000', '환경설정', G5_ADMIN_URL . '/config_form.php', 'config'));
    for ($i = 0; $admin_row = sql_fetch_array($admin_menu); $i++) {
       array_push( $menu['menu100'],array($admin_row['sub_menu'], $admin_row['menu_title'], G5_ADMIN_URL .$admin_row['menu_url'], ''));
    }
}
