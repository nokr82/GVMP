...
if ($csv == 'csv')
{
    ...
    $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_name, od_b_tel, od_b_hp, b.it_name, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.ct_send_cost, b.it_sc_type FROM ";
	if($is_admin=="ad") {
		$sql .= "(SELECT * FROM g5_shop_order WHERE od_id IN (SELECT DISTINCT od_id FROM g5_shop_cart WHERE it_id IN (SELECT DISTINCT it_id FROM g5_shop_item WHERE mb_id = '{$member['mb_id']}'))) a , {$g5['g5_shop_cart_table']} b ";
	}else{
		$sql .= "{$g5['g5_shop_order_table']} a, {$g5['g5_shop_cart_table']} b ";
	}
	$sql .= " where a.od_id = b.od_id ";
    if ($case == 1) // 출력기간
