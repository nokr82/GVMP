...
<div class="local_ov01 local_ov">
    ...
    <span class="btn_ov01"><span class="ov_txt">등록된 상품</span><span
                class="ov_num"> <?php echo $total_count; ?>건</span></span>
</div>
...
<form name="flist" class="local_sch01 local_sch">
    ...
    <select name="sca" id="sca">
        ...
        <?php
        $sql1 = " select ca_id, ca_name from {$g5['g5_shop_category_table']}";
        if ($is_admin == "ad") {
            $sql1 .= " WHERE ca_id like '90%'";
        }
        $sql1 .= " order by ca_order, ca_id ";
...
<form name="fitemlistupdate" method="post" action="./itemlistupdate.php" onsubmit="return fitemlist_submit(this);"
      autocomplete="off" id="fitemlistupdate">
    ...
    <div class="tbl_head01 tbl_wrap">
        <table>
            ...
            <tr>
                ...
                <th scope="col" id="th_pt">신청제품관리</th>
            ...
            for ($i = 0; $row = sql_fetch_array($result); $i++) {
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td colspan="5" class="td_sort">
                        <label for="ca_id_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?>
                            기본분류</label>
                        <select name="ca_id[<?php echo $i; ?>]" id="ca_id_<?php echo $i; ?>">
                            <?php echo conv_selected_option($ca_list, $row['ca_id']); ?>
                        </select>
                        <label for="ca_id2_<?php echo $i; ?>"
                               class="sound_only"><?php echo get_text($row['it_name']); ?> 2차분류</label>
                        <select name="ca_id2[<?php echo $i; ?>]" id="ca_id2_<?php echo $i; ?>">
                            <?php echo conv_selected_option($ca_list, $row['ca_id2']); ?>
                        </select>
                        <label for="ca_id3_<?php echo $i; ?>"
                               class="sound_only"><?php echo get_text($row['it_name']); ?> 3차분류</label>
                    ...
                    <td rowspan="3" class="td_num">
                        ...
                        <input type="text" name="it_order[<?php echo $i; ?>]" value="<?php echo $row['it_order']; ?>"
                               id="order_<?php echo $i; ?>" class="tbl_input" size="3">
                    </td>
                    <td rowspan="3">
                        <label for="use_<?php echo $i; ?>" class="sound_only">판매여부</label>
                        <input type="checkbox"
                               name="it_use[<?php echo $i; ?>]" <?php echo($row['it_use'] ? 'checked' : ''); ?>
                               value="1" id="use_<?php echo $i; ?>">
                    </td>
                    <td rowspan="3">
                    ...
                    <td rowspan="3" class="td_mng td_mng_s">
                        <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>"
                           class="btn btn_03"><span
                        ...
                        if ($is_admin == "super" || $is_admin == "manager") {
                            ...
                            <a href="./itemcopy.php?it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>"
                               class="itemcopy btn btn_02" target="_blank"><span
                                        class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'], 250, "")); ?> </span>복사</a>
                        <?
                        } ?>
                        <a href="<?php echo $href; ?>" class="btn btn_02"><span
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td headers="th_pc_title" rowspan="2" class="td_input">
                        ...
                        <input type="text" name="it_name[<?php echo $i; ?>]"
                               ...
                               id="name_<?php echo $i; ?>" required class="tbl_input required" size="30">
                    </td>
                    <td headers="th_amt" class="td_numbig td_input">
                        <label for="price_<?php echo $i; ?>" class="sound_only">판매가격</label>
                        <input type="text" name="it_price[<?php echo $i; ?>]" value="<?php echo $row['it_price']; ?>"
                               id="price_<?php echo $i; ?>" class="tbl_input sit_amt" size="7">
                    </td>
                    <td headers="th_camt" class="td_numbig td_input">
                        ...
                        <input type="text" name="it_cust_price[<?php echo $i; ?>]"
                               ...
                               class="tbl_input sit_camt" size="7">
                    </td>
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td headers="th_pt" id="btn_pd_wr" class="td_numbig td_input">
                        <?php
                        if ($row['ca_id'] == "10") {
                            if ($row['view'] != "YES") {
                                echo "<a class=\"btn_add_1 btn_pd\" id=\"{$row['it_id']}\">추가</a>";
                            } else {
                                echo "<a class=\"btn_del_1 btn_pd\" id=\"{$row['it_id']}\">삭제</a>";
                            }
                        }
                        ?>
                    ...
                    <td headers="th_qty" class="td_numbig td_input">
                        ...
                        <input type="text" name="it_stock_qty[<?php echo $i; ?>]"
                               ...
                               class="tbl_input sit_qty" size="7">
                    </td>
    ...
    <div class="btn_fixed_top">
        ...
        <?php if ($is_admin == 'super' || $is_admin == "manager") { ?>
            <a href="./itemexcel.php" onclick="return excelform(this.href);" target="_blank"
               class="btn btn_02">상품일괄등록</a>
...
<script>
    ...
    //제품관리 추가삭제 이벤트
    $(function () {
        $("#btn_pd_wr > a").click(function () {
            var myId = $(this).attr("id");

            if ($(this).hasClass('btn_add_1')) {
                if (confirm("상품코드  : " + myId + "번을 추가하시겠습니까?") == true) {    //확인
                    window.location.href = "./itemlistBack.php?type=add&it_id=" + myId;
                } else {   //취소
                    return;
                }
            }

            if ($(this).hasClass('btn_del_1')) {
                if (confirm("상품코드  : " + myId + "번을 삭제하시겠습니까?") == true) {    //확인
                    window.location.href = "./itemlistBack.php?type=del&it_id=" + myId;
                } else {   //취소
                    return;
                }
            }
