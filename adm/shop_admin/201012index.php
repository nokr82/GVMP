...
$g5['title'] = ' 쇼핑몰관리';
include_once (G5_ADMIN_PATH.'/201012admin.head.php');
echo G5_ADMIN_PATH;
...
function get_order_status_sum($status)
{
	...
	if($is_admin=="super"||$is_admin=="manager"){
		$sql .= " from {$g5['g5_shop_order_table']} where ";
	}else{
		$sql .= " FROM g5_shop_order WHERE od_id IN (SELECT DISTINCT od_id FROM g5_shop_cart WHERE it_id IN (SELECT DISTINCT it_id FROM g5_shop_item WHERE mb_id = '{$member['mb_id']}')) and ";
	}
	$sql .= " od_status = '$status' ";
    $row = sql_fetch($sql);
...
function get_order_date_sum($date)
{
    ...
    $sql .= " SUBSTRING(od_time, 1, 10) = '$date' ";
    $row = sql_fetch($sql);
...
?>
<link href="/theme/everyday/css/200923mobile_shop.css">