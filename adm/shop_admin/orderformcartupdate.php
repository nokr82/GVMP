...
for ($i=0; $i<$cnt; $i++)
{
	...
	if($ct_status=="완료"){
		//cash지급
		//# 상품 구매시
		//1. 구매자에게 1%(구매자의 계정 종류와 상관없이)
		//2. 구매자가 SM, CU, VD일 경우 구매자의 추천인에게도 1%(추천인이 리뉴얼 기간이여야 지급)
		//3. 판매자의 추천인이 VM이면 1%(추천인이 리뉴얼 기간이여야 지급)
		//cash지급
		$userpoint = (int)$ct['ct_price']*0.01;
        sql_query("update g5_member set VCash = VCash + {$userpoint} where mb_id = '{$ct['mb_id']}'");
		sql_query("insert into dayPoint set mb_id = '{$ct['mb_id']}', VCash = +{$userpoint}, date = NOW(), way = 'shopPointCashBack'");

		$arow = sql_fetch("SELECT accountType FROM g5_member WHERE mb_id = '".$ct['mb_id']."'");
		$accountType = $arow['accountType'];
		$grow = sql_fetch("SELECT recommenderID FROM genealogy WHERE mb_id = '".$ct['mb_id']."'");
		$geneid = $grow['recommenderID'];

		$prow = sql_fetch("SELECT recommenderID FROM genealogy WHERE mb_id = (SELECT mb_id FROM temp_item_shop WHERE it_id = '".$ct['it_id']."')");
		$pmbid = $prow['recommenderID'];
		$prow = sql_fetch("SELECT accountType FROM g5_member WHERE mb_id = '".$pmbid."'");
		$paccountType = $prow['accountType'];

		if($accountType=="SM"||$accountType=="CU"||$accountType=="VD"){
			$crow = sql_fetch("SELECT count(*) as cnt FROM g5_member WHERE mb_id='".$geneid."' AND DATE_ADD(renewal, INTERVAL +1 MONTH) >= '".date("Y-m-d")."'");
			if($crow['cnt']>0){
				sql_query("update g5_member set VCash = VCash + {$userpoint} where mb_id = '{$geneid}'");
				sql_query("insert into dayPoint set mb_id = '{$geneid}', VCash = +{$userpoint}, date = NOW(), way = 'shopPointCashBack'");
			}
		}
		if($paccountType=="VM") {
			$crow = sql_fetch("SELECT count(*) as cnt FROM g5_member WHERE mb_id='".$pmbid."' AND DATE_ADD(renewal, INTERVAL +1 MONTH) >= '".date("Y-m-d")."'");
			if($crow['cnt']>0){
				sql_query("update g5_member set VCash = VCash + {$userpoint} where mb_id = '{$pmbid}'");
				sql_query("insert into dayPoint set mb_id = '{$pmbid}', VCash = +{$userpoint}, date = NOW(), way = 'shopPointCashBack'");
			}
		}
...
//push처리
$message = "주문번호 {$od_id}의 상품이 {$ct_status}되었습니다.";
$push_token = getMemmberPushToken($mb_id);
$result = Android_Chat_Push($push_token, "주문알림", $message,"alram", "", "");
if($result) {
	savePushContent("주문알림","주문",$message,$mb_id,$result,$ct_status);
}
//push처리
