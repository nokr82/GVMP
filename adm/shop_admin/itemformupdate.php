...
if ($w == '' || $w == 'u') {
	auth_check($auth[$sub_menu], "d",false,"ad");
...
if ($_POST['sale_mb_id'] != ''){
    $sql = "select * from g5_member where mb_id = '{$_POST['sale_mb_id']}'";
    $sale_member = sql_fetch($sql);
    if (!$sale_member){
        alert("존재하지 않는 판매자아이디입니다.");
        exit();
    }
...
$sql_common = " ca_id               = '$ca_id',
                ...
                it_9                = '$it_9',
                it_10               = '$it_10',
                mb_id               = '{$_POST['sale_mb_id']}'
                ";
if($is_admin=="ad")$sql_common .= ", mb_id = '{$member['mb_id']}'";
if ($w == "")
{
    $it_id = $_POST['it_id'];
    ...
    // 상품 등록시 VMP 직원한테 문자 발송
    $mb_hp_vmp = "010-7357-5230";
    ...
    $strData = urlencode("상품등록\n상품코드:{$it_id}");
    getFromUrl("http://gvmp.company/sms/sms.php?strData={$strData}&strTelList={$mb_hp_vmp};&strCallBack=07074515121");
