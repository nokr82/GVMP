...
if($is_admin=="super"||$is_admin=="manager"){
	$sql .= " {$g5['g5_shop_order_table']} where ";
}else{
	$sql .= " g5_shop_order WHERE od_id IN (SELECT DISTINCT od_id FROM g5_shop_cart WHERE it_id IN (SELECT DISTINCT it_id FROM g5_shop_item WHERE mb_id = '{$member['mb_id']}')) AND ";
}
$sql .= "  SUBSTRING(od_time,1,7) between '$fr_month' and '$to_month'
