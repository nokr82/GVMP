...
if ($P_vmc > 0 || $P_vmr > 0 || $P_vmp > 0 || $P_vmg > 0 || $P_vmm > 0) {
    ...
    $sql = "type="checkbox" onclick="levelall(this)" adminpay(day_point_no,admin_id,type) value ('{$point_id}','{$mb_id}',1)";
    sql_query($sql);
}

if ($M_vmc > 0 || $M_vmr > 0 || $M_vmp > 0 || $M_vmg > 0 || $M_vmm > 0) {
    sql_query("type="checkbox" onclick="levelall(this)" dayPoint(mb_id,VMC,VMR,VMP,VMM,VMG,date,way) value ('{$mb_id}',-{$M_vmc},-{$M_vmr},-{$M_vmp},-{$M_vmm},-{$M_vmg},NOW(),'admin2')");
    $point_id = sql_insert_id();
    $sql = "insert into adminpay(day_point_no,admin_id,type) value ('{$point_id}','{$mb_id}',2)";
    sql_query($sql);
}
...
goto_url('./member_form_category.php?' . $qstr . '&amp;w=u&amp;mb_id=' . $mb_id . "&check=true", false);
?>