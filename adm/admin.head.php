...
function print_menu2($key, $no='')
{
    ...
    {
        if ($is_admin != 'super' && $is_admin != 'ad' && $is_admin != 'manager' && (!array_key_exists($menu[$key][$i][0],$auth) || !strstr($auth[$menu[$key][$i][0]], 'r')))
            continue;
...
<header id="hd">
    ...
    <div id="hd_top" onmousedown="mouseDown()">
        <button type="button" id="btn_gnb" class="btn_gnb_close <?php echo $adm_menu_cookie['btn_gnb'];?>">메뉴</button>
       <div id="logo"><a href="<?php echo G5_ADMIN_URL ?>/shop_admin/"><img src="<?php echo G5_ADMIN_URL ?>/img/logo.png" alt="<?php echo $config['cf_title'] ?> 관리자"></a></div>

       <script>
       $('#hd').bind("taphold", function()
        {
         alert("Long Click");
        });
        </script>
