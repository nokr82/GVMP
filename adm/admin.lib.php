...
// 권한 검사
function auth_check($auth, $attr, $return = false, $check = null)
{
    global $is_admin;
    if ($check && $is_admin == $check)
        return;
    if ($is_admin == 'super')
        return;
    if ($is_admin == 'manager')
...
if (!$member['mb_id']) {
...
} else if ($is_admin != 'super' && $is_admin != 'ad' && $is_admin != 'manager') {
    $auth = array();
    ...
    if (!$i) {
        if ($_SERVER['HTTP_HOST'] . $request_uri != 'gvmp.company/adm/shop_admin/0717orderlist.php' && !strpos($_SERVER['HTTP_HOST'] . $request_uri,'0717orderform.php')){
