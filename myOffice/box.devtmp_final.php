...
body {padding:20px;}
  ...
  i {font-style:normal;}
  c {display:block;position:relative;width:150px;margin-bottom:50px;height:40px;border:1px solid;flex-shrink:0;margin-right:5px;text-align:center;box-sizing:border-box;line-height:34px;background-color: #edc951;}
  c:after {content:" ";position:absolute;left:50%;top:39px;width:2px;height:16px;border-left:1px solid;} 
  g {position:absolute;left:50%;width:310px;height:34px;margin-top:-35px;border:1px solid;border-bottom:0;}  
  .lv1  g {display:none;}
  c:nth-child(even)  g {display:none;}  
c.sad {background:#75dcf6;}
c.angry {background:#e56a77;}
.fa-stack {position:absolute;top:-10px;left:-10px;width:30px;}
.fa-stack i {font-size:30px;}

.fa-circle {color:#edc951}
c.sad .fa-circle {color:#75dcf6}
c.angry .fa-circle {color:#e56a77}

.fa-emotion:before {content: "\f581";}
c.sad .fa-emotion:before {content: "\f5b4";}
c.angry .fa-emotion:before {content: "\f5c8";}
d {
position: absolute;
    top: -10px;
    left: -10px;
...
echo "<div class='scale_all'><div class='wrap_all'>";
		...
		for($j=1;$j<=$deCount;$j++){
			if($arrRow){
				foreach($arrRow as $key=>$val){
					...
					if($j == $val['seq']){
						echo "<c><d><e></e><f></f></d><i onclick='dm(".$val['mb_id'].", ".$val['sponsorID'].")'>".$val['mb_name']."</i><g></g></c>";
					}else if(!in_array($j,$seqArr) && $midx == $val['mb_id']){
						echo "<c class='n'>없음<g></g></c>";
...
<script type="text/javascript">
   $(function(){
   	var idx = $('.wrap').length;
   	var lastItemlength = $('.LV' + idx + ' c').length;
   	var lastWidth = lastItemlength * 155;
    $('.wrap').width(lastWidth);
    for (var i = 2; i <= idx; i++) {
    	 $('.LV' + i + ' g').css('width', $('.Lv' + i).children('c:first-child').next().offset().left - $('.Lv' + i).children('c:first-child').offset().left);
    }
...
window.onload=function(){
	...
	//
        ...
        MOBILEBOOL = false;
		$(".wrap_all").drags();
		$('.scale_all').mousewheel(function(event, deltaY) {
