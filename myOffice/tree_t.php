...
<script>
    ...
    $(function () {
        //현재페이지가 초기페이지와같으면
        var uri = '<?=$request_uri?>';
        if (result != 1 && uri == '/myOffice/tree_t.php') {
            start_load();
        }
    });
    ...
    function excel_down() {
        ...
        $('.loading_txt').html('엑셀을 다운로드중 입니다.');
        $(".loading_wrap").addClass('on');
        var f = document.exel_down;
        f.action = "./tree_exel.php";
        f.method = "post";
        f.submit();
    ...
    function searchClick() {
        if (result != 1 &&  '<?=$request_uri?>' == '/myOffice/tree_t.php') {
            loading();
        ...
        if ($("#nameNick option:selected").val() == "아이디") {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == "true") {
                        location.href = '/myOffice/tree_t.php?mb_id=' + $('#searchInput').val();
                    } else {
...
//                       submenu.slideDown();
    ...
    function searchSelect(id) {
        window.location.href = '/myOffice/tree_t.php?mb_id=' + id;
    }
</script>
