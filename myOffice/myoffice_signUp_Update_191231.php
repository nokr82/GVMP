...
$USERID				= (isset($_POST['mb_id'])&&trim($_POST['mb_id']))?trim($_POST['mb_id']):"";
$USERNAME			= (isset($_POST['mb_name'])&&trim($_POST['mb_name']))?trim($_POST['mb_name']):"";
...
function insertMember($USERID,$USERNAME,$TEAMARRAY,$PUSER,$MEMTOTAL)
{
    ...
    foreach($TEAMARRAY as $key => $value){
        $result = mysql_query("select mb_no from {$MEMBERTABLE} order by mb_no desc limit 1");
        $temp2;
