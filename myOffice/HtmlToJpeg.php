...
class HtmlToJpeg
{
    ...
    {
        $this->config = array(
            'folder' => '/var/www/html/gvmp/myOffice/uploads/',
    ...
    {
        $output = '<form name="'.$this->config['form_id'].'" id="' . $this->config['form_id'] . '" action="' . $this->config['action_url'] . '" method="POST">';

        foreach ($this->html_contents as $i => $htm) {
            $output .= '<div class="' . $this->config['content_class_name'] . '" id="page-' . $i . '">' . $htm . '</div>';
        }

        $output .= '<button type="submit" onClick="thisForm()">' . $this->config['download_button_label'] . '</button>';
        $output .= '</form>';

        $output .= "<script>";
        $output .= '   var hidden_image_names = "' . $this->config['hidden_image_names'] . '";';
        $output .= '   var content_class_name = "' . $this->config['content_class_name'] . '";';
        $output .= '   var form_id = "' . $this->config['form_id'] . '";';
        $output .= "</script>";
