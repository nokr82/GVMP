...
<div class="pop_companymap">
    ...
    <div class="wrap_pop">
        <form method="post" action="./update_test.php" id="fregisterform" name="fregisterform" onsubmit="return checkIt()">
            ...
            <h4>회원정보 변경</h4>
            <table valign="middle" cellspacing='0'> 
                <tr class='desktop'>
                    <th><label for="">휴대폰 번호</label></th>
                    <td class="wt">
                ...
                <tr>
                  ...
                  <tr>
                    ...
                    <td colspan="3"><input type="email" id="email" name="email" value="<?= $mb_row['mb_email'] ?>"  placeholder="email@naver.com"/></td>                            
                </tr>
                <tr>
                    <th><label for="">비밀번호</label></th>
                    <td colspan="3"><input type="password" id="passwd" name="passwd" required="required"/></td>                            
                </tr>
                <tr>
                    <th><label for="">비밀번호 확인</label></th>
                    <td colspan="3"><input type="password" id="con_passwd" name="con_passwd" required="required"/></td>                            
                </tr>
                <tr>
                    <th valign='top' class='address_pd'><label for="">주소</label></th>
                    <td colspan="3">
                        <label for="reg_mb_zip" class="sound_only">우편번호</label>
                        <input type="text" required="required"  id="mb_zip" name="mb_zip" value="<?= $mb_row['mb_zip1'] . $mb_row['mb_zip2'] ?>"  class="frm_input " size="5" maxlength="6" placeholder="우편번호">
                        ...
                        <input type="text" required="required" name="mb_addr1" value="<?= get_text($mb_row['mb_addr1']) ?>" id="mb_addr1" class="wt2" size="50" placeholder="주소를 검색해주세요.">
                        <label for="mb_addr2"  class="sound_only">상세주소</label>
                        <input type="text" required="required" class="wt2" name="mb_addr2" value="<?= get_text($mb_row['mb_addr2']) ?>" id="mb_addr2" size="50" placeholder="주소를 검색해주세요.">
                    </td>
                <input type="hidden" name="mb_addr3" value="<?php echo get_text($mb_row['mb_addr3']) ?>" id="mb_addr3" class="frm_input frm_address" size="50" readonly="readonly" placeholder="참고항목">
                <input type="hidden" name="mb_addr_jibeon" id="mb_addr_jibeon" value="<?php echo get_text($mb_row['mb_addr_jibeon']); ?>">
                </tr>
                <tr class='desktop'>
                    <th><label for="">은행명</label></th>
                    <td><input type="text" required="required" id="bankName" name="bankName" value="<?= $mb_row['bankName'] ?>" placeholder="은행명"/></td>
                    <th><label for="">예금주</label></th>
                    <td class="wt1"><input type="text" required="required" id="accountHolder" name="accountHolder" value="<?= $mb_row['accountHolder'] ?>" placeholder="예금주"/></td>
                </tr>

                <tr>
                    <th><label for="">계좌번호</label></th>
                    <td colspan="3"><input type="text" required="required" id="accountNumber" name="accountNumber" value="<?= $mb_row['accountNumber'] ?>" placeholder="계좌번호"/></td> 
                </tr>                 
...
<!--
              ...
              <tr class='mobile'>
                ...
                <tr class='mobile'>
                    <th><label for="">인증번호</label></th>
                    <td colspan="3">    <td class="wt1"><input type="text" maxlength="4" id="con_num" placeholder="인증번호를 입력해주세요."/></td>
                </tr>
                <tr class='mobile'>
                    <th><label for="">은행명</label></th>
                    <td colspan="3"><input type="text" required="required" id="bankName" name="bankName" value="<?= $mb_row['bankName'] ?>" placeholder="은행명"/></td>
                </tr>
                <tr class='mobile'>
                    <th><label for="">예금주</label></th>
                    <td colspan="3"><input type="text" required="required" id="accountHolder" name="accountHolder" value="<?= $mb_row['accountHolder'] ?>" placeholder="예금주"/></td>
                </tr>-->
            </table>  
