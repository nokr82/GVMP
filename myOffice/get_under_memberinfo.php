...
if ($sql_search != '' || ($sql_type != '' && $sql_level != '')) {
    $sql = "SELECT t.*,m.mb_email,m.mb_hp,m.mb_datetime FROM genealogy_tree as t INNER JOIN g5_member as m ON t.mb_id = m.mb_id WHERE rootid = '{$id}' {$sql_search} {$sql_level} {$sql_type} order by t.mb_id asc limit {$p_num}, {$page}";
