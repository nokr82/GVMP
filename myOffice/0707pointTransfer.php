...
if ($_POST['last_id'] == ''){
    ...
    $query = mysql_query("select * from smsCode where sms_code = '{$_POST['smsinnumber']}' and n = '{$_POST['last_id']}'");
    $sms_code = mysql_fetch_array($query);
    if ($sms_code){
        $sql = "DELETE FROM smsCode WHERE n = '{$_POST['last_id']}'";
        mysql_query($sql);
    }else{
        alert("경고!!! 잘못된 경로입니다.");
        exit();
