        ...
        if ($("#nameNick option:selected").val() == "아이디") {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == "true") {
                        location.href = '/myOffice/sales_1.php?mb_id=' + $('#searchInput').val();
                    } else {
...
//                       submenu.slideDown();
                    for (var i = 0; i < json.length; i++) {
                        $('#search_info_tbl > tbody:last').append('<tr id=\"' + json[i][1] + '\" onclick=\"searchSelect(this.id);\"><td>' + json[i][0] + '</td><td>' + json[i][2] + '</td><td>' + json[i][5] + '</td><td>' + json[i][3] + '</td><td>9999-99-99</td></a></tr>');
                    }
                }
            });
        }
    ...
    function searchSelect(id) {
        window.location.href = '/myOffice/sales_1.php?mb_id=' + id;
    }
