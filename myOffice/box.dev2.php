...
<!-- 컨텐츠 -->
    <section class="boxSection clearfix">
        ...
        <div class="boxSearBox">
            ...
            <?php
                if( $lower1T_ID != "" ) {
                    echo "<a href=\"/myOffice/box_1.php?mb_id={$lower1T_ID}#\" class=\"boxTree_btn boxTree_btn2\">1팀 최하단</a>";
                } else {
                    echo "<a href=\"#\" class=\"boxTree_btn boxTree_btn2\">1팀 최하단</a>";
                }

                if( $lower2T_ID != "" ) {
                    echo "<a href=\"/myOffice/box_1.php?mb_id={$lower2T_ID}#\" class=\"boxTree_btn boxTree_btn3\">2팀 최하단</a>";
                } else {
...
<!--        <ul class="preview">
        	...
        	<li><a href="#"><img src="images/replay.png" alt="" onclick="selectView('<?=$member['mb_id']?>')";></a></li>
                    	...
                    	<?php
                        	...
                        	echo "<img src=\"{$imagePath}\" onerror=\"this.style.display='none'\" ></div><p>이름 : {$row['mb_name']}<br>ID : {$row['mb_id']}<br />가입 : {$row['mb_open_date']}<br /><font style=\"color: {$renewalColor};\">리뉴얼 : {$timestamp}</font><br />인원 : {$re[0]}/$re[1]/{$row2['team3']}<br />추천 : {$row['recommenderName']}<br /></p></a><div class=\"boxLine\"></div></div></div>";
                    	?>
