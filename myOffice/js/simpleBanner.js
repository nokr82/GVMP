...
//		nav_wrap.css("left", (W - nav_wrap.width())/2 + "px" );
	...
	//	setTimeout("sbAnimateBannerTimer("+i+")", itv);  가운데 191224 롤링배너 막기
		
		// init events
		// wrap.bind("mouseenter", sbMouseEnterListener);  가운데 191224 롤링배너 막기
		// wrap.bind("mouseleave", sbMouseLeaveListener);  가운데 191224 롤링배너 막기
	});
}
...
function sbAnimateBanner(wrap, cur){
	...
	if(type == "hslide"){
		wrap_ul.stop(true);
		wrap_ul.animate({left:-W*cur+"px"}, 800);
	}else if(type == "vslide"){
		wrap_ul.stop(true);
		wrap_ul.animate({top:-H*cur+"px"}, 800);
	}else{
		var li;
