...
(function($) {
    ...
    $.fn.orgChart.defaults = {
        ...
        copyTitle  : true,
        replace    : true,
		renewState : function(str) {return $(str).attr("alt");}
    };
    ...
    function buildNode($node, $appendTo, level, index, opts) {
		...
		var objTxt = opts.nodeText($node);
		var customCalss = opts.renewState(objTxt);

        var $heading = $("<h2>").html(objTxt);
        var $nodeDiv = $("<div>").addClass("node").addClass("level"+level).addClass("node"+index).addClass("level"+level+"-node"+index).addClass(customCalss).append($heading);
