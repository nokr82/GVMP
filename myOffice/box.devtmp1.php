	...
	echo "<div class='wrap lv".$i."'>";
		for($j=1;$j<=$deCount;$j++){
				foreach($arrRow as $key=>$val){
					...
					if($j == $val['seq']){
                                           $plusTemp = "";
                                            if( $val['1T_ID'] != "" || $val['2T_ID'] != "" ) {
                                                $plusTemp = "(+)";
                                            }
						echo "<div class='item ' data-idx='".$val['mb_id']."'  data-pIdx = '".$val['sponsorID']."'><span  class='fa-stack fa-2x'><i class='fas fa-circle fa-stack-2x'></i><i class='far fa-emotion fa-stack-2x'></i></span><i onclick=\"detailMemberModal('".$val['mb_id']."', '".$val['sponsorID']."')\">".$val['mb_name']."{$plusTemp}</i><span class='line_tree'></span></div>";
