...
<link rel="stylesheet" href="./css/index_start.css"/>
...
if ($imageAdRow["image1"] != "") {
    $imageTempCheck = true;
    if ($imageAdRow["mb_id"] == "00202327") {
        if (rand(1, 10) != 1) {
            $imageTempCheck = false;
        }
    }
...
<!--                    <li class="bg5">
    ...
    <form action="0707pointTransfer.php" method="POST" id="pointTransferF">
        ...
        <div id="pop_container3" style="position: fixed; visibility: hidden;">
            <div id="con_fix3" class="popup_position">
                ...
                <div class="card1 cash"><a href="javascript:void(0)" onClick="smscodeconf()">포인트 전송</a></div>
    ...
    <div class="wrap_playkok_idconnect">
        ...
        <div class="wrap_playkok_login">
            ...
            <div class="wrap_input_kok">
                ...
                <button type="button" onclick="play_kok_form()" id="login_btn" class="login_btn">계정연동</button>
            </div>
    ...
    <script type="text/javascript">
        ...
        function play_kok_form() {
            ...
            var vmp_id = $('#vmp_id').val();
            var api_email = $('#api_email').val();
            var api_pw = $('#api_pw').val();

            $.ajax({
                type: 'post',
                url: 'play_kok.php',
                data: {'vmp_id': vmp_id, 'api_email': api_email, 'api_pw': api_pw},
                // dataType: 'json',
    ...
    <div id="Point_Calculation">
        ...
        <script>
            ...
            var last_id = 0; //문자인증번호 아이디값

            function smscodedel(idx) {
                $.ajax({
                    type: 'post',
                    url: '0701smscodedel.php',
                    data: {'sms_id': idx},
                    dataType: 'json',
                    error: function (xhr, status, error) {
            ...
            function smscodeconf() {
                ...
                $.ajax({
                    type: 'post',
                    url: '0701smscodeconf.php',
                    data: {'sms_id': last_id, 'sms_code': sms_code},
                    dataType: 'json',
                    error: function (xhr, status, error) {
                        alert(error + xhr + status);
                    },
    ...
    <form action="0701update.php" method="post" name="myForm" id="myForm">
        <input type="hidden" id="sms_code" name="sms_code" value="">
        <input type="hidden" id="u_last_id" name="u_last_id">
                ...
                success: function (data) {
                    if (data.success == 'ok') {
                        ...
                        $(obj).hide(function () {
                            ...
                            $(confirmBox).append('<input type="text" name="hpinnumber" id="hpinnumber" autocomplete="off"  maxlength="6" value=""/>');
                            $(confirmBox).append('<p class="hpinnumber_text" style="display:none">인증번호가 틀립니다.(6자리)</br>다시 입력해주세요</p>');
...
<script>
   // console.log('현재 VM 기간이 만료된 상태입니다. 리뉴얼을 진행하셔야 VM 권한이 유지됩니다2222323232.');
</script>

</body>
</html>
<?php
//include_once('../shop/201012shop.tail.php');
...
if ($member['accountType'] == 'VM' && ($now_temp > $timestamp_temp && $now_temp <= $timestamp2_temp)) {
    ...
    <script>
        ...
        alert('현재 VM 기간이 만료된 상태입니다. 리뉴얼을 진행하셔야 VM 권한이 유지됩니다.');
    </script>
    <?php
}
...
<script>
//찾다가 못 찾아서 결국 수동으로 지웁니다
$('link').each(function(){
    var $linkplease = $(this).attr('href') == 'http://gvmp.company/theme/everyday/mobile/skin/shop/basic/style.css?ver=171222';
    var $linkplease2 = $(this).attr('href') == 'http://gvmp.company/theme/everyday/css/mobile_shop.css?ver=171222';
