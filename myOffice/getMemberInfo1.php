...
$POSITION = 0;
//이곳이다 성지다
function jsonPrintTreeDetail1($tree)
{
    ...
    if (!is_null($tree) && count($tree) > 0) {
        ...
        foreach ($tree as $key => $node) {
            ...
            jsonPrintTreeDetail1($node['children']);
            $DATAHTML .= '</li>';
        }
        $DATAHTML .= '</ul>';
    }
    ...
    $row2 = mysql_fetch_array(mysql_query("SELECT 
        ...
        WHERE
            G.recommenderID = '{$mb_id}'
                AND DATE_ADD(M.renewal, INTERVAL + 1 month) >= CURDATE()"));

    $rank = "";
    if ($row2["C"] >= 3 && $row["team1"] >= 3000 && $row["team2"] >= 3000) {
        if (rankCountCheck($mb_id, 'Crown AMBASSADOR', 1)) {
            $rank = "Royal Crown AMBASSADOR";
        } else {
            $rank = "Crown AMBASSADOR";
        }
    } else if ($row2["C"] >= 3 && $row["team1"] >= 2000 && $row["team2"] >= 2000) {
        $rank = "Triple AMBASSADOR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 1000 && $row["team2"] >= 1000) {
        $rank = "Double AMBASSADOR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 600 && $row["team2"] >= 600) {
        $rank = "AMBASSADOR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 375 && $row["team2"] >= 375) {
        $rank = "5 STAR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 250 && $row["team2"] >= 250) {
        $rank = "4 STAR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 150 && $row["team2"] >= 150) {
        $rank = "3 STAR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 75 && $row["team2"] >= 75) {
        $rank = "2 STAR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 40 && $row["team2"] >= 40) {
        $rank = "1 STAR";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 20 && $row["team2"] >= 20) {
        $rank = "Triple MASTER";
    } else if ($row2["C"] >= 3 && $row["team1"] >= 6 && $row["team2"] >= 6) {
        $rank = "Double MASTER";
    } else if ($row2["C"] >= 3) {
        $rank = "MASTER";
    } else {
...
function rankCountCheck($mb_id, $accountRank, $N)
{
    ...
    if ($check1["orderNum"] <= $rankOrderByRow["orderNum"]) {
        $teamCount[0]++;
    }
    if ($check2["orderNum"] <= $rankOrderByRow["orderNum"]) {
        $teamCount[1]++;
    }
