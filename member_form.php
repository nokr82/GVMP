...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        <th scope="row"><label for="mb_register_num">주민등록번호</label></th>
        <td><input type="text" name="mb_register_num" value="<?=$mb["RRN"]?>" id="mb_register_num" class="frm_input" size="15" maxlength="20"></td>
    </tr>
<!--    <tr>
...
<!--    <tr>
    ...
    <tr>
        <th scope="row">추천인</th>
        <td colspan="3">
            <input type="text" name="mb_recom_id" id="mb_recom_id" class="frm_input" value="<?php echo ($mb['mb_recommend'] ? get_text($mb['mb_recommend']) : '없음'); // 081022 : CSRF 보안 결함으로 인한 코드 수정 ?>">
        </td>
    </tr>
    <?php } ?>
...
this.form.mb_intercept_date.value=this.form.mb_intercept_date.defaultValue; }">
    ...
    <tr>
        <th>
            <label for="rank_hotplayer">핫플레이어</label>
        </th>
        <td>
            <input class="frm_input" type="text" name="rank_hotplayer" id="rank_hotplayer" value="<?=$mb["hotplayer"]?>" placeholder="1" onkeyup="onlyNumber(this)">
            ※ 0을 입력하면 순위에서 제외됩니다.
        </td>
    </tr>
    ...
    <tr>
        <th>
            <label for="rank_hotplayer">버니 등급</label>
        </th>
        <td>
            <input class="frm_input" type="text" name="accountRank" id="rank_hotplayer" value="<?=$mb["accountRank"]?>" maxlength="1" onkeyup="onlyNumber(this)">
            ※ 기본 2등급이며 1등급 또는 3등급으로 변경할 수 있습니다. 1~3 외에 다른 수를 입력하지 마시오.
        </td>
    </tr>
    ...
    <?php
        $pointRow1 = mysql_fetch_array(mysql_query("select sum(point) as p from pointTBL where mb_id = '{$mb["mb_id"]}'"));
        $pointRe2 = mysql_query("select point-withdrawPoint as p, datetime from pointTBL where mb_id = '{$mb["mb_id"]}' and point != withdrawPoint and (payType = '카드 결제' or payType = '네이버 결제')");
        $pointRow3 = mysql_fetch_array(mysql_query("select sum(point-withdrawPoint) as p from pointTBL where mb_id = '{$mb["mb_id"]}' and point != withdrawPoint and ((payType != '카드 결제' and payType != '네이버 결제')or payType is null)"));
        $pointRow2Total = 0;
    ...
    <tr>
        <th>
            <label for="change_business">누적 포인트</label>
        </th>
        <td>
            <?php echo number_format($pointRow1["p"]); ?>
        </td>
    </tr>
    <tr>
        <th>
            <label for="change_business">보유 포인트</label>
        </th>
        <td>
            <?php echo number_format($mb["point"]);?>
        </td>
    </tr>
    <tr>
        <th>
            <label for="change_business">인출가능 포인트</label>
        </th>
        <td>
            <?php echo number_format($pointRow2Total+$pointRow3["p"]); ?>
        </td>
    </tr>
     ...
     <tr>
        <th>
            <label for="pay_point">포인트 지급</label>
        </th>
        <td>
            <input class="frm_input" type="text" name="pay_point1" id="pay_point" value="" onkeyup="onlyNumberComma(this)" placeholder="금액">
            <input class="frm_input" type="text" name="pay_point2" id="pay_point" value="" placeholder="내용">
            ※ 해당 회원에게 포인트가 지급됩니다.
        </td>
    </tr>
    <tr>
        <th>
            <label for="pay_point">포인트 감소</label>
        </th>
        <td>
            <input class="frm_input" type="text" name="pay_point3" id="pay_point" value="" onkeyup="onlyNumberComma(this)" placeholder="금액">
            <input class="frm_input" type="text" name="pay_point4" id="pay_point" value="" placeholder="내용">
            ※ 해당 회원에게 포인트가 차감됩니다.
        </td>
    </tr>
    <tr>
        <th>
            <label for="change_business">버니 여부</label>
        </th>
        <td>
            <input class="frm_input" type="checkbox" name="change_bunny" id="change_bunny" value="Y" <?php if( $mb["accountType"] == "버니") {echo "checked='true'";} ?>>
            ※ 체크하시면버니로 전환되고 체크 해제시 일반으로 전환됩니다.
        </td>
    </tr>
    <tr>
        <th>
            <label for="change_business">사업자 여부</label>
        </th>
        <td>
            <input class="frm_input" type="checkbox" name="change_business" id="change_business" value="Y" <?php if( $mb["buisnessman"] == "Y") {echo "checked='true'";} ?>>
            ※ 체크하시면 사업자 버니로 전환됩니다.
        </td>
    </tr>
...
<script>
    alertMsg('success', '수정되었습니다.', function() {alertOff();} );
</script>
<?php
    }
?>