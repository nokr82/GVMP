...
sql_query("update g5_member set vmc = vmc - {$_POST['vmc']}, vmr = vmr - {$_POST['vmr']}, vmp = vmp - {$_POST['vmp']} where mb_id = '{$_POST['loginID']}'");
sql_query("insert into dayPoint set mb_id = '{$_POST['loginID']}', VMC=-{$_POST['vmc']}, VMR=-{$_POST['vmr']}, VMP=-{$_POST['vmp']}, date=NOW(), way = 'vmJoin;{$temp2}'");

sql_query("insert into dayPoint set mb_id = '{$temp2}', VMC=0, VMR=0, VMP=0,V=0,VCash=0,VPay=0, date=NOW(), way = 'vmJoin2'");

sql_query("insert into teamMembers values(null, '{$temp2}', null, null, null, null, null, null)");
...
if ($mb_sponsor_no == "99999999") {
    sql_query("insert into team3_list values(null, '{$mb_recommender_no}', '{$mb_name}', '{$temp2}')");
} else {
    // 유료 회원가입이라면 teamMembers DB 업데이트 시키기
    sql_query("update teamMembers set {$radiobtn}T_name = '{$mb_name}', {$radiobtn}T_ID = '{$temp2}' where mb_id = '{$mb_sponsor_no}'");
    sql_query("insert into plusCountListTBL set mb_id = '{$temp2}'"); // VM 카운팅 리스트(증가)
}
...
$result = sql_query("select * from g5_member where mb_id = '{$temp2}'");
