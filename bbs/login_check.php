...
if (strpos($_POST['mb_id'], "@")) {
    $re = sql_fetch("select * from g5_member where mb_email = '{$_POST['mb_id']}'");
    $_POST['mb_id'] = $re['mb_id'];
} else if (strpos($_GET['mb_id'], "@")) {
    $re = sql_fetch("select * from g5_member where mb_email = '{$_GET['mb_id']}'");
    $_GET['mb_id'] = $re['mb_id'];
}

$mb_idx = (isset($_POST['mb_idx']) && $_POST['mb_idx']) ? $_POST['mb_idx'] : $_GET['mb_idx'];

if ($mb_idx) {
    $mb = app_get_memmber($mb_idx);
    set_session('app_it_id', $it_id);
    set_session('app_mb_idx', $mb_idx);
    $_SESSION['user_id'] = $mb['mb_id'];
    $mb_password = trim($mb['mb_password']);
    $g5['title'] = "로그인 검사";
} else {
    $mb_id2 = trim($_GET['mb_id']);
    $_SESSION['user_id'] = $_POST['mb_id'];
    $g5['title'] = "로그인 검사";
    $mb_id = trim($_POST['mb_id']);
    $mb_password = trim($_POST['mb_password']);
    if ($mb_id2 == null && (!$mb_id || !$mb_password))
        alert('회원아이디나 비밀번호가 공백이면 안됩니다.');
    if ($mb_id2 != null) {
        $mb = get_member($mb_id2);
    } else {
        $mb = get_member($mb_id);
    }
}
...
if ($mb_idx) {
    if (!$is_social_password_check && (!$mb['mb_id'] || !check_password2($mb_password, $mb['mb_password'])) && $mb_id2 == null) {
        alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다.');
    }
} else {
    if (!$is_social_password_check && (!$mb['mb_id'] || !check_password($mb_password, $mb['mb_password'])) && $mb_id2 == null) {
        alert('가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다.');
    }
}
...
// 비밀번호 세션 생성
set_session('ss_mb_password', $mb_password);
...
//인증한시간입력  동우추가 0605
if ($_POST['ct_sms'] == 'Y') {
    $sql = " update {$g5['member_table']} set mb_cert = now() where mb_id = '{$mb['mb_id']}' ";
    sql_query($sql);
}
...
if ($mb_idx) {
    ...
    //동우추가 0206
    if ($_GET['mb_id'] != '') {
        goto_url("/myOffice/index.php");
    } else {
        goto_url("/bbs/board.php?bo_table=blog");
    }
