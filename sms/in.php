...
<body onload="setDate()">
<div id="phone_body">
  <form id="MsgForm" name="MsgForm" method="POST" action="./sms.php" onSubmit="return varcheck();">
  <input type="hidden" name="strTelList" value="<?=$_POST['strTelList']?>">
  ...
  <div class="text">
      <textarea name="strData" class="sendsms" onchange="ChkLen()" onkeyup="ChkLen()" rows="7" cols="20" value="<?=$_POST['reg_mb_recommender']?>"></textarea></div>
  <!-- 전송창 -->
  <!-- 바이트표시 -->
  <div class="byte">
  ...
  <div class="Numb">
      ...
      보낸번호 <input type="text" name="strCallBack" value="<?=$_POST['strcallBack']?>">
  </div>
  <!-- //받는번호 보내는번호 -->
  <div class="line"></div>
  <!-- 즉시전송 예약전송선택 -->
