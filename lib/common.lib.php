...
function print_r2($var)
{
    ...
    ob_end_clean();
    $str = str_replace(" ", "&nbsp;", $str);
    echo nl2br("<span style='font-family:Tahoma, 굴림; font-size:12pt;'>$str</span></br>");
}
function P($var)
{
    ob_start();
...
function get_member($mb_id, $fields='*')
{
    ...
    return sql_fetch(" select $fields, DATE_ADD(renewal, INTERVAL + 1 MONTH) as renewal_1, DATE_ADD(renewal, INTERVAL + 2 MONTH) as renewal_2 from {$g5['member_table']} where mb_id = TRIM('$mb_id') or mb_email = TRIM('$mb_id') ");
}

function app_get_memmber($mb_idx, $fields='*') 
{
	global $g5;
	return sql_fetch(" select $fields from {$g5['member_table']} where mb_no = TRIM('$mb_idx') ");
}
...
{
    global $config, $group, $board;
    $level = get_member($mb_id, 'mb_level');
    if (!$mb_id) return;
    if ($level['mb_level'] == '9') return 'manager';
...
//    if (member_ad_check($mb_id)) return 'ad'; // VD를 관리자로 인정하는 로직
...
function member_ad_check($mb_id) {
    global $g5;

    $sql = " select accountType from {$g5['member_table']} where mb_id= '".$mb_id."'";
    $mb = sql_fetch($sql);
	if($mb['accountType']=="VD"){
		return true;
	}else{
		return false;
	}
...
function getRenewalDate($MBID,$AType)
{
	$row = sql_fetch("SELECT count(*) as cnt FROM g5_member WHERE mb_id='".$MBID."' AND DATE_ADD(renewal, INTERVAL +1 MONTH) >= '".date("Y-m-d")."'");
	$data = $row['cnt'];
...
function Android_Push($tokens, $title, $message, $kind="", $room="") {
    $url = 'https://fcm.googleapis.com/fcm/send';
    $apiKey = "AIzaSyA3HSivtwYSmJAh_bM7XHXcCZYQrds-ttc";
	if($kind=="chat") {
		$clickAction = "ChatingActivity";
	}else{
		$clickAction = "AlramActivity";
	}
    $fields = array('to' => $tokens, "data" => array("title"=>$title, "content"=>$message, "kind"=>$kind, "Room" => $room, "clickAction" => $clickAction));
...
function Android_Chat_Push($tokens, $title, $message, $kind="", $room="", $toid) {
    $url = 'https://fcm.googleapis.com/fcm/send';
    $apiKey = "AIzaSyA3HSivtwYSmJAh_bM7XHXcCZYQrds-ttc";
	if($kind=="chat") {
		$clickAction = "ChatingActivity";
		$sql = "SELECT title,subtitle FROM usersChatingRoom WHERE idx = '" .$room. "'";
		$row = sql_fetch($sql);
		$title = $row['title'];
		$image = get_member_profile_img_app($toid);
	}else if($kind == "logout"){
		$clickAction = "LOGOUT";
	}else{
		$clickAction = "AlramActivity";
	}
    $fields = array('to' => $tokens, "data" => array("title"=>$title, "content"=>$message, "kind"=>$kind, "Room" => $room, "clickAction" => $clickAction, "image" => $image));
    $headers = array('Authorization:key='.$apiKey,'Content-Type: application/json');
...
function get_member_profile_img_app($mb_id=''){
    global $member;
    ...
    if( $mb_id ){
		$member_img = G5_DATA_PATH.'/member_image/'.$mb_id.'/'.$mb_id.'.gif';
		if (is_file($member_img)) {
			$member_cache[$mb_id] = $src = str_replace(G5_DATA_PATH, G5_DATA_URL, $member_img);
		}
    }
...
function getMemmberPushToken($MBID) {
	$sql = "SELECT Token FROM usersPush WHERE mb_id = '".$MBID."'";
	$row = sql_fetch($sql);
	return $row['Token'];
}


function getPushContent($content,$od_name,$od_b_name,$od_id,$totalprice,$mb_id,$company)
{
	$content = str_replace("{이름}", $od_name, $content);
	$content = str_replace("{보낸분}", $od_name, $content);
	$content = str_replace("{받는분}", $od_b_name, $content);
	$content = str_replace("{주문번호}", $od_id, $content);
	$content = str_replace("{주문금액}", number_format($totalprice), $content);
	$content = str_replace("{회원아이디}", $mb_id, $content);
...
function savePushContent($title,$state,$content,$mb_id,$push_result)
{
	$sql = "INSERT INTO usersPushList set Content = '".strip_tags($content)."', Title = '".$title."', mb_id = '".$mb_id."', state='".$state."',pushResult='".$push_result."',indate='".date("Y-m-d H:i:s")."'";
	sql_query($sql);
}
