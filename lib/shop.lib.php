        ...
        if ($this->query) {
            ...
            if ($this->ca_id || $this->ca_id2 || $this->ca_id3) {
                ...
                if ($caidtmp) {
					if(strlen($caidtmp)>2) {
					    //문제
//	                    $where_ca_id[] = " ca_id2 like '".substr($caidtmp,0,3)."%'";
                        $where_ca_id[] = " ca_id2 like '".substr($caidtmp,0,4)."%'";
					}else{
	                    $where_ca_id[] = " ca_id = '{$this->ca_id}' ";
					}
                }
                if ($this->ca_id2) {
            ...
            $sql_where = " where it_1 = '' and " . implode(" and ", $where);
...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    ...
    for($i=1;$i<=10; $i++) {
		if(strpos($row['it_img'.$i], "ttp://")){
			$imgtmp = '<img src="'.$row['it_img'.$i].'" width="'.$width.'" height="'.$height.'"/>';
		}else{
...
// sns 공유하기
function get_sns_share_link($sns, $url, $title, $img,$state=null)
{
    global $config;
...
//            if($config['cf_kakao_js_apikey'])
                $str = '<a href="javascript:kakaolink_send(\''.str_replace('+', ' ', urlencode($title)).'\', \''.urlencode($url).'\');" class="share-kakaotalk"><img src="'.$img.'" alt="카카오톡 링크보내기">';
				if($state)$str .= '공유하기';
				$str .= '</a>';
            break;
    }
...
// 상품 추가옵션
function app_get_item_supply($it_id, $subject, $is_div='')
{
    global $g5;
...
//위도 경도
function getAddrToTitude($addr=''){
	$access_token = 'e4373e8fd4088f0a8c811ed754bd965b';
	$params = sprintf('?query=%s', urlencode($addr));
	...
	curl_setopt($s, CURLOPT_POSTFIELDS, $info['query']);
	curl_setopt($s, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($s, CURLOPT_HTTPHEADER, array('Authorization: KakaoAK ' . $access_token));
...
// APP 상품 이미지를 얻는다
function get_app_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    global $g5;
	...
	if($imgtmp){
		...
		if($thumb) {
			...
			$img = '<img src="'.G5_SHOP_URL.'/img/no_image.gif" alt="'.$img_alt.'"';
		}
...
function imageFileRotationCheck($file) {
	$source_path = $_SERVER['DOCUMENT_ROOT'] . $file;
