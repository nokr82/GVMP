...
if(!defined('G5_IS_ADMIN') && defined('G5_THEME_PATH') && is_file(G5_THEME_PATH.'/head.sub.php')) {
    require_once(G5_THEME_PATH.'/200923head.sub.php');
    return;
}
...
<?php
//검색안되게하는 메타태그 20200128동우추가
echo '<meta name="robots" content="noindex, nofollow">'.PHP_EOL;


if (G5_IS_MOBILE) {
    echo '<meta name="viewport" content="initial-scale=1.0,user-scalable=no,maximum-scale=1.0,width=device-width" />'.PHP_EOL;
...
if ($is_member) { // 회원이라면 로그인 중이라는 메세지를 출력해준다.
    ...
    else if ($is_admin == 'manager') $sr_admin_msg = "부관리자";
