...
$_POST["mb_register_num"] = trim($_POST["mb_register_num"]);
...
// 포인트 지급 로직
if( $_POST["pay_point1"] != "" && $_POST["pay_point2"] != "" ) {
    $_POST["pay_point1"] = preg_replace("/[^0-9]/", "",$_POST["pay_point1"]);
    $_POST["pay_point2"] = trim($_POST["pay_point2"]);
    sql_query("update g5_member set point = point + {$_POST["pay_point1"]} where mb_id = '{$mb_id}'") or die("ERROR");
    sql_query("insert into pointTBL set mb_id = '{$mb_id}', point = {$_POST["pay_point1"]}, datetime = now(), way = '{$_POST["pay_point2"]}', withdrawPoint = 0") or die("ERROR");
}

// 포인트 감소 로직
if( $_POST["pay_point3"] != "" && $_POST["pay_point4"] != "" ) {
    $_POST["pay_point3"] = preg_replace("/[^0-9]/", "",$_POST["pay_point3"]);
    $_POST["pay_point4"] = trim($_POST["pay_point4"]);
    sql_query("update g5_member set point = point - {$_POST["pay_point3"]} where mb_id = '{$mb_id}'") or die("ERROR");
    sql_query("insert into pointTBL set mb_id = '{$mb_id}', point = -{$_POST["pay_point3"]}, datetime = now(), way = '{$_POST["pay_point4"]}', withdrawPoint = 0") or die("ERROR");
    sql_query("insert into pointTBL set mb_id = '{$mb_id}', point = {$_POST["pay_point3"]}, datetime = now(), way = '누적용', withdrawPoint = {$_POST["pay_point3"]}") or die("ERROR");
}


if( $_POST["change_bunny"] == "Y" ) {
    $_POST["change_bunny"] = "버니";
} else if( $_POST['change_bunny'] != "Y" ) {
    $_POST['change_bunny'] = "일반";
}

if( $_POST["change_business"] != "Y" ) {
    $_POST["change_business"] = "N";
}
if( $_POST['rank_hotplayer'] == "" ) {
    $_POST['rank_hotplayer'] = 0;
}


$recooCheck = sql_fetch_array(sql_query("select * from g5_member where mb_id = '{$_POST['mb_recom_id']}'"));
$recoSQL = "";
if( $recooCheck["mb_id"] != "" ) {
    $recoSQL = "mb_recommend = '{$_POST['mb_recom_id']}', ";
}
...
$sql_common = "  mb_name = '{$_POST['mb_name']}',
                 ...
                 mb_9 = '{$_POST['mb_9']}',
                mb_10 = '{$_POST['mb_10']}',
                 RRN = '{$_POST["mb_register_num"]}',
                 accountRank = '{$_POST["accountRank"]}',
                 accountType = '{$_POST['change_bunny']}',
                 buisnessman = '{$_POST["change_business"]}',
                 {$recoSQL}
                 hotplayer = {$_POST['rank_hotplayer']} ";
