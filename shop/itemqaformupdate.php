...
if ($w == "")
{
	...
	$sql = "SELECT Token,mb_id FROM usersPush WHERE mb_id = (SELECT mb_id FROM {$g5['g5_shop_item_table']} WHERE it_id = '".$it_id."')";
	$row = sql_fetch($sql);
	$result = Android_Chat_Push($row['Token'], "상품문의", $alert_msg,"alram", "", "");
	if($result) {
		savePushContent("상품문의","상품문의",$alert_msg,$row['mb_id'],$result);
	}
}
else if ($w == "u")
{
