...
if ((int)($row['od_price'] - $tot_cp_price) !== $i_price) {
    if( $_POST['settle_method'] != "vmp_point" ) {
        die("Error.");
    }
}
...
if ((int)((int)$send_cost - (int)$tot_sc_cp_price) !== (int)((int)$i_send_cost - (int)$i_send_coupon)) {
    die("Error..");
}
//////홍동우  0604 제주도 산간지방 배송비문제
// 추가배송비가 상이함
...
//동우수정 배송비문제 0626
//if((int)$send_cost2 !== (int)$i_send_cost2) {
if((int)$tmp['sc_price'] !== (int)$i_send_cost2) {
	die("{$tmp['sc_price']}-----{$i_send_cost2} 배송비오류");
}
...
else if ($od_settle_case == "KAKAOPAY")
{
    ...
    if($od_misu == 0)
        $od_status      = '입금';
} else if($od_settle_case == "vmp_point" || $_POST['settle_method'] == "vmp_point") {
    $od_settle_case = "vmp_point";
    $od_tno             = $tno;
    $od_receipt_price   = $amount;
    $od_receipt_point   = $i_temp_point;
    $od_receipt_time    = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})/", "\\1-\\2-\\3 \\4:\\5:\\6", $app_time);
    $od_bank_account    = "vmp_point";
    $pg_price           = $amount;
    $od_misu            = 0;
...
//동우추가 0630 판매자만의 주문내역남기기
include_once(G5_SHOP_PATH.'/ordermail3.inc.php');
...
// VMC, VMP, VCash, VPay 포인트를 사용했다면 보유 포인트에서 차감시키기
if(isset($_POST['vmcF']) || isset($_POST['vmpF']) || isset($_POST['vmmF']) || isset($_POST['vcashF']) || isset($_POST['vpayF'])) {
	if( (int)$_POST['vmcF'] > 0 || (int)$_POST['vmpF'] > 0 || (int)$_POST['vmmF'] > 0 || (int)$_POST['vcashF'] > 0 || (int)$_POST['vpayF'] > 0 ) {
		if($_SESSION['app']){
                    sql_query("update g5_member set VCash = VCash - {$_POST['vmcF']}, VPay = VPay - {$_POST['vmpF']} where mb_id = '{$member['mb_id']}'");
                    sql_query("insert into dayPoint set mb_id = '{$member['mb_id']}', VCash = -{$_POST['vmcF']}, VMR = 0, VPay = -{$_POST['vmpF']}, date = NOW(), way = 'shopPoint'");
		}else{
                    sql_query("update g5_member set VMC = VMC - {$_POST['vmcF']}, VMP = VMP - {$_POST['vmpF']}, VMM = VMM - {$_POST['vmmF']}, VCash = VCash - {$_POST['vcashF']}, VPay = VPay - {$_POST['vpayF']} where mb_id = '{$member['mb_id']}'");
                    sql_query("insert into dayPoint set mb_id = '{$member['mb_id']}', VMC = -{$_POST['vmcF']}, VMP = -{$_POST['vmpF']}, VMM = -{$_POST['vmmF']}, VCash = -{$_POST['vcashF']}, VPay = -{$_POST['vpayF']}, date = NOW(), way = 'shopPoint'");
		}
	}
}
...
/*판매자가 admin이 아닐 때 판매자의 추천인에게 결제 금액에 1% VCash 지급
//구매자가 accountType이 VM이면 결제 금액에 1% VCash 지급
//구매자가 accountType이 CU, VD, SM이면 구매자의 추천인에게 1% VCash 지급
//총 2명에게 각 각 1% 씩 지급.
*****/

if($od_status=="입금" && $od_receipt_price){ //입금이 되었고 결제 금액이 있을때 동작
	$sql = "SELECT *, (SELECT mb_id FROM g5_shop_item WHERE it_id = gc.it_id) AS vd_mb_id FROM g5_shop_cart AS gc WHERE gc.od_id = '".$od_id."'";

	$result = sql_query($sql);
	while($row=sql_fetch_array($result)){
		if($row['vd_mb_id']&&$row['ct_status']=="입금"&&$row['vd_mb_id']<>"admin") { //입력한 VD아이디가 있고 입금 처리가 되었고 admin이 아닐때
			$ct_price = $row['ct_price']; //결제 금액
			$crow = sql_fetch("SELECT recommenderID FROM genealogy WHERE mb_id = '".$row['vd_mb_id']."'");
			$sellRecom = $crow['recommenderID']; // 판매자의 추천인
			$recomPrice = $ct_price * 0.01; //결재 금액의 1%

			if($sellRecom) {
				if(getRenewalDate($sellRecom)){ //renewal 날짜 체크
					sql_query("update g5_member set VCash = VCash + {$recomPrice} where mb_id = '{$sellRecom}'");
					sql_query("insert into dayPoint set mb_id = '{$sellRecom}', VCash = +{$recomPrice} date = NOW(), way = 'vendorSellPoint'");
				}
			}

			if($member['accountType']=="VM"){	// 구매자에게 1% VCash 지급
				if(getRenewalDate($member['mb_id'])){ //renewal 날짜 체크
					sql_query("update g5_member set VCash = VCash + {$recomPrice} where mb_id = '{$member['mb_id']}'");
					sql_query("insert into dayPoint set mb_id = '{$member['mb_id']}', VCash = +{$recomPrice} date = NOW(), way = 'vendorBuyPoint'");
				}
			}else if($member['accountType']=="CU"||$member['accountType']=="VD"||$member['accountType']=="SM"){ //구매자의 추천인에게 1% VCash 지급
				$crow = sql_fetch("SELECT recommenderID FROM genealogy WHERE mb_id = '".$member['mb_id']."'");
				$buyRecom = $crow['recommenderID']; // 구매자의 추천인
				if($buyRecom){
					if(getRenewalDate($buyRecom)){ //renewal 날짜 체크
						sql_query("update g5_member set VCash = VCash + {$recomPrice} where mb_id = '{$buyRecom}'");
						sql_query("insert into dayPoint set mb_id = '{$buyRecom}', VCash = +{$recomPrice} date = NOW(), way = 'vendorRecomPoint'");
					}
				}
			}
		}
...
//if($od_status=="입금" && $od_receipt_price){ //입금이 되었고 결제 금액이 있을때 동작
/*주문한 회원에게*/
	$push_contents = $default['de_sms_cont2'];
	$push_token = getMemmberPushToken($member['mb_id']);
	$totalpricetmp = $tot_ct_price + $od_send_cost + $od_send_cost2;
	$message = getPushContent($push_contents,$od_name,$od_b_name,$od_id,$totalpricetmp,$member['mb_id'],$default['de_admin_company_name']);

	if($push_token){
		$result = Android_Chat_Push($push_token, "결제알림", $message,"alram", "", "");
		if($result) {
			savePushContent("결제알림","결제",$message,$member['mb_id'],$result);
		}
	}
/*주문한 회원에게*/

/*각 상품의 VD에게*/
	$push_contents = $default['de_sms_cont3'];
	$totalpricetmp = $tot_ct_price + $od_send_cost + $od_send_cost2;
	$message = getPushContent($push_contents,$od_name,$od_b_name,$od_id,$totalpricetmp,$member['mb_id'],$default['de_admin_company_name']);
	...
	$result = sql_query($sql);
	while($row=sql_fetch_array($result)){
		if($row['vd_mb_id']) { //VD mb_id가 있을때
			$push_token = getMemmberPushToken($row['vd_mb_id']);
			$result = Android_Chat_Push($push_token, "결제알림", $message,"alram", "", "");
			if($result) {
				savePushContent("결제알림","결제VD",$message,$row['vd_mb_id'],$result);
			}
		}
	}
/*각 상품의 VD에게*/
//}
if($od_settle_case == '무통장' && $od_misu > 0) { //무통장일떄 회원에게
	$push_contents = $od_name."님의 입금계좌입니다.\n금액:".number_format($od_misu)."원\n계좌:".$od_bank_account."\n".$default['de_admin_company_name'];
	$push_token = getMemmberPushToken($member['mb_id']);
	$message = getPushContent($push_contents,$od_name,$od_b_name,$od_id,$totalpricetmp,$member['mb_id'],$default['de_admin_company_name']);

	if($push_token){
		$result = Android_Chat_Push($push_token, "무통장알림", $message,"alram", "", "");
		if($result) {
			savePushContent("무통장알림","무통장",$message,$member['mb_id'],$result);
		}
	}
}
///////////
