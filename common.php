...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    $sale_member = sql_fetch("SELECT count(*) as cnt FROM gyc5.temp_item_shop where mb_id = '{$member['mb_id']}'");
...
if(isset($config['cf_theme']) && trim($config['cf_theme'])) {
	if($_SESSION['app']){
		$theme_path = "/var/www/html/gvmp/theme/vexmall";
		$config['cf_theme'] = "vexmall";
	}else{
